...
-- to show the orphan rows in the child table------
SELECT 
	c.salesmanId ,
	s.salesmanId
FROM Customer c
LEFT JOIN  Salesman s ON s.SalesmanId = c.SalesmanId 
WHERE c.SalesmanId IS NOT NULL AND s.SalesmanId IS NULL;


-------------Need to remove the orphan rows ---------------
DELETE FROM Customer
WHERE SalesmanId  NOT IN (SELECT SalesmanId FROM Salesman) ;

SELECT * FROM Customer ;
--==========Now perform the foreign key constraint ==============
